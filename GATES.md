...
- [x] G10: The popover, the settings window and the app icon render as specified
  EVIDENCE: Screenshots taken from the signed bundle running out of /Applications. Popover: 340pt wide, header is the 14pt Lucide globe-check glyph plus "Imperator DefaultBrowser" in .headline, one row per browser, each carrying a switch on the right, the default row (Safari) filled #A01818 with its switch on, Google Chrome, Firefox and cmux with theirs off, rows 6pt apart, footer reads Open at Login / Settings / About / Quit. The on switch sits on a dark track rather than a brand-red one, because a red track on the red row would hide the control in its own background. Hover: with the pointer parked on a row, that row takes a white 8% fill and the others stay flat. Measured from the capture rather than eyeballed, the hovered row samples r=0.311 against r=0.203 on an unhovered one, and the brand-red default row is left alone on purpose. Cursor: captured with screencapture -C so the pointer is drawn. With the pointer on the Google Chrome switch the cursor is the pointing hand, not an arrow. That is the whole reason the switch is two drawn shapes rather than a Toggle: the first build used a real Toggle and the same capture showed an arrow, because NSSwitch installs its own cursor rect below SwiftUI and allowsHitTesting(false) does not remove it. Settings window: 400x480, "Browser Order" plus the drag hint, drag handles on every row, Safari carrying the brand-red DEFAULT capsule and every other row a white "Set as default" action, footer "Scan for Browsers" / "Reset Order" / the browser count. Nothing in that window overrides the cursor, so it stays on the system pointer throughout. App icon: NSWorkspace.icon(forFile:) on /Applications/Imperator DefaultBrowser.app renders the delivered artwork at 1024x1024, a black squircle with the violet Imperator sigil, and the .icns inside the installed bundle is byte-identical to Resources/AppIcon.icns (md5 8cbbdebb5f6aab05592f8b90d4602550).
